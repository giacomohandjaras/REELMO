...
if(kHat>-0.5)
    ...
    if(critical_value-q)>0
        ...
        if (pvalue<eps/2)
            if(debug>0);disp(sprintf('The pvalue is beyond single precision...'));end
        ...
        if(debug>0); disp(sprintf('The pvalue is outside the ight tail...'));end
    ...
    position=find(null_distro_sort==critical_value);
    pvalue=position(1)/(permutations+1);
    if(debug>0); disp(sprintf('kHat is too low for a Pareto, I will keep the original p-value of %.6f',pvalue));end
end



%%%%Let's evaluate the effect at the specified p-value
if(debug>0); disp(sprintf('Measure the critical value for the pvalue %.6f...',critical_p));end
